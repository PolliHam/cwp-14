...
const actfilm = require('./data/actfilm.json');
...
(async () => {
    ...
    await db.actors.bulkCreate(actors.slice(1,5));
    await db.actorfilms.bulkCreate(actfilm.slice(1,4));
    ...
    (await db.films.findByPk(1, {//связываем 2 табл
        include: [{
            model: db.actors,
            as: 'Actors'
        }]
...
//хуки это технология перевата вызовов функций в чужих процессах.
//суть - заставит программу поверить, что нужня ей функци находится в другом месте.
 //некое словие которое должно выполниться
